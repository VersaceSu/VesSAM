...
def build_sam_vit_h(args):
    return _build_sam(
        ...
        encoder_adapter=args.adapter_train,
    )
...
def _build_ves_sam(norm_layer, args):
    model = Ves_SAM(
        image_encoder=ImageEncoderViT(
            img_size=args.image_size,
            patch_size=args.vit_batch_size,
            embed_dim=args.encoder_embed_dim,
            in_chans=args.in_chans,

            depth=args.encoder_depth,
            qkv_bias=args.qkv_bias,
            norm_layer=norm_layer,  # 如果需要要从这里改,这个是特殊的不在args参数里面的
            ...
            out_chans=args.encoder_out_dim,
            adapter_train=args.adapter_train,
        ),
        prompt_encoder=PromptEncoder(
            embed_dim=args.prompt_embed_dim,
            base_chans=args.base_chans,
            img_size=args.image_size,
            activation=nn.GELU,  # **************如果需要要从这里改,这个是特殊的不在args参数里面的
            num_heads=args.prompt_num_heads,
            num_layers=args.prompt_num_layers,
            graph_layers=args.prompt_graph_layers,
            point_emb_chans=args.point_emb_chans
        ),
        mask_decoder=MaskDecoder(
            num_multimask_outputs=args.num_multimask_outputs,
            transformer=TwoWayTransformer(
                depth=args.mask_trans_depth,
                embedding_dim=args.encoder_out_dim,
                mlp_dim=args.mask_mlp_dim,
                num_heads=args.mask_num_heads,
            ),
            transformer_dim=args.mask_transformer_dim,
            iou_head_depth=args.iou_head_depth,
            iou_head_hidden_dim=args.iou_head_hidden_dim,
        ),
        pixel_mean=[123.675, 116.28, 103.53],
        pixel_std=[58.395, 57.12, 57.375],
    )
    if args.checkpoint is not None and args.use_ckpt:
        with open(args.checkpoint, "rb") as f:
            state_dict = torch.load(f, map_location="cpu")
        try:
            if 'model' in state_dict.keys():
                # print(args.adapter_train)
                model.load_state_dict(state_dict['model'], False)
            else:
                if args.image_size == 1024 and args.adapter_train == True:
                    model.load_state_dict(state_dict, False)
                else:
                    model.load_state_dict(state_dict)
        except:
            print('*******interpolate')
            new_state_dict = load_from(model, state_dict, args.image_size, args.vit_batch_size)
...
def load_from(sam, state_dicts, image_size, vit_patch_size):
    ...
    # sam_dict = {k: v for k, v in sam_dict.items() if k.startswith("image_encoder.")}
    ...
    new_state_dict = {
        k: v for k, v in state_dicts['model'].items()
