...
class MaskDecoder(nn.Module):
    ...
    def predict_masks(
        ...
        # dense_prompt_embeddings = torch.repeat_interleave(dense_prompt_embeddings, src.shape[0]//dense_prompt_embeddings.shape[0], dim=0)
        src = src.repeat_interleave(2, dim=2).repeat_interleave(2, dim=3)
        src = src + dense_prompt_embeddings  # 添加 dense 嵌入
        # image_pe = image_pe.repeat_interleave(2, dim=2).repeat_interleave(2, dim=3)
        ...
        # upscaled_embedding = []
        # for i, upscaling_layer in enumerate(self.output_upscaling):
        #     src = upscaling_layer(src)
        #     upscaled_embedding.append(src)
...
if __name__ == '__main__':
    ...
    # 创建输入张量
    batch_size = 4
    image_embedding = torch.randn(batch_size, 256, 32, 32)  # 图像嵌入
    image_pe = torch.randn(1, 256, 64, 64)  # 图像位置编码
