...
class CrossAttentionTransformer(nn.Module):
    ...
    def forward(self, dense_embedding: torch.Tensor, sparse_embedding: torch.Tensor) -> tuple[
        ...
        dense_embeddings = dense_embedding.permute(0, 2, 3, 1)
        batch_size, H, W, embed_dim = dense_embeddings.shape
        dense_embeddings_new = dense_embeddings.view(batch_size, -1, embed_dim)

        for attn_layer in self.attn_layers:
            sparse_embeddings, _ = attn_layer(sparse_embedding, dense_embeddings_new, dense_embeddings_new)
            dense_embeddings_new, _ = attn_layer(dense_embeddings_new, sparse_embeddings, sparse_embeddings)

        return sparse_embeddings + sparse_embedding, dense_embeddings_new
