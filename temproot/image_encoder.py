...
class ImageEncoderViT(nn.Module):
    def __init__(
        ...
        """
                图像编码器 ViT (Vision Transformer)
                Args:
                    img_size (int): 输入图像尺寸。
                    patch_size (int): 输入图像的 patch 尺寸。
                    in_chans (int): 输入图像的通道数。
                    embed_dim (int): 嵌入维度。
                    depth (int): Transformer 的深度。
                    num_heads (int): 注意力头的数量。
                    mlp_ratio (float): MLP 隐藏层维度与输入嵌入维度的比率。
                    qkv_bias (bool): 是否为 QKV 添加偏置。
                    norm_layer (nn.Module): 正则化层。
                    act_layer (nn.Module): 激活函数。
                    use_abs_pos (bool): 是否使用绝对位置编码。
                    use_rel_pos (bool): 是否使用相对位置编码。
                    rel_pos_zero_init (bool): 是否初始化相对位置编码为 0。
                    window_size (int): 用于窗口注意力的窗口大小。
                    global_attn_indexes (Tuple[int, ...]): 使用全局注意力的层索引。
                    adapter_train (bool): 是否启用适配器训练。
                """
        super().__init__()
